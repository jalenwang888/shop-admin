<script setup>
  ...
  const getItem = (to) => {
    reportStore.fetchReports(perPage.value, to, 'new').then(response => {
    ...
    <VDataTable v-model:items="reports" :items-per-page=perPage show-select :search="searchQuery" :headers="headers"
      ...
      <template #item.user_id="{ item }">
        <div class="d-flex align-center gap-4">
          <RouterLink :to="{ name: 'reports-view-id', params: { id: item.raw.id } }" class="font-weight-medium user-list-name" >
      ...
      <template #item.status="{ item }">
        <VChip
                :color="statusVariant(item.raw.status)"
                size="small"
                label
